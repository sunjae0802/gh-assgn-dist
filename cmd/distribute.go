...
var distributeTemplate string
var distributeClassroom string
var distributeDryRun bool
var distributeOnly string

var DistributeCmd = &cobra.Command{
	Use:   "distribute ASSGN",
	Short: "Distribute an assignment as per-student repos",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		assgn := args[0]

		classroomFile := distributeClassroom
		if classroomFile == "" {
		...
		for _, u := range unmatched {
			fmt.Printf("warning: %q from --only is not in the roster\n", u)
		}
		if len(students) == 0 {
...
// filterStudents keeps only the students whose GitHub username appears in the
// comma-separated only list, matching case-insensitively because GitHub
// usernames are. An empty list selects everyone. It also reports the entries of
// the list that matched no student, which usually means a typo.
func filterStudents(students []internal.Student, only string) ([]internal.Student, []string) {
	...
	var selected []internal.Student
	matched := map[string]bool{}
	for _, s := range students {
		key := strings.ToLower(s.GitHub)
		if wanted[key] {
...
func init() {
	DistributeCmd.Flags().StringVar(&distributeTemplate, "template", "", "Template repo (owner/name); defaults to ORG/ASSGN")
	DistributeCmd.Flags().StringVar(&distributeClassroom, "classroom", "", "Classroom YAML file (default \"classroom.yaml\")")
	DistributeCmd.Flags().BoolVar(&distributeDryRun, "dry-run", false, "Print gh api commands without executing")
	DistributeCmd.Flags().StringVar(&distributeOnly, "only", "", "Comma-separated GitHub usernames to distribute to (default: everyone in the roster)")
}
